...
<div class="formbody">
    <div  class="usual">
        <div class="tabson">
            <form id="addgoodsForm" method="post" ENCTYPE="multipart/form-data" action="${pageContext.request.contextPath}/admin/good/update">
                <ul class="forminfo">
                    <input type="hidden" name="id" value="${good.id}">
                    <li>
                        <label>商品缩略图<b>*</b></label>
                        <img src="${pageContext.request.contextPath}${good.picture}" height="60px" width="80px"/>
                        <input name="picture" id="picture" type="file"  multiple="multiple"/>

                    </li>
                    <li>
                        <label>商品名称<b>*</b></label>
                        <input name="goodName" id="goodName" type="text" class="dfinput" value="${good.goodName}"
                    ...
                    <li>
                        ...
                        <input name="goodPrice" id="goodPrice" value="${good.goodPrice}" class="dfinput" type="number"  required="required"  style="width:100px;"
                               step="1" min="0" onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''"
                        />元 </li>
                    <li>
                        <label>库存数量<b>*</b></label>
                        <input name="goodStore" id="goodStore" value="${good.goodStore}" type="number" class="dfinput"  required="required" style="width:100px;"
                               step="1" min="0" onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''"
                        />
                        件 </li>
                    <li>
                        <label>商品详情<b>*</b></label>
                        <textarea name="detail" rows="3" id="content"   style="width:500px;height:100px;">${good.detail}</textarea>
                    </li>
                    <li>  <label>&nbsp;</label>
                        <input type="submit" class="btn" id="btnPublish" value="修改"/>
                    </li>
                </ul>
            </form>
        </div>
